...
async def authenticate_user(email: str, password: str) -> dict[str, Any]:
    ...
    locked_until = _coerce_utc(user.get("locked_until"))
    if locked_until is not None and locked_until > _utcnow():
        logger.warning("Login blocked by lockout for email=%s locked_until=%s", normalized_email, locked_until)
        raise HTTPException(
    ...
    if not verify_password(password, str(user.get("hashed_password", ""))):
        ...
        update_fields: dict[str, Any] = {"failed_login_attempts": failed_attempts, "updated_at": _utcnow()}
        if failed_attempts >= FAILED_LOGIN_LIMIT:
            ...
            logger.warning(
                "Account locked after %s failed attempts email=%s for %s minutes",
                FAILED_LOGIN_LIMIT, normalized_email, LOCKOUT_MINUTES,
            )
        try:
            await users_collection.update_one({"_id": user["_id"]}, {"$set": update_fields})
        except PyMongoError:
            logger.exception("Failed to persist failed-login counter for email=%s", normalized_email)
