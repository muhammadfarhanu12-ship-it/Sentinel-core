...
# TODO: move this to a Mongo collection (e.g. blocked_ips_collection) so IPs can be
# added/removed without a redeploy. Kept as a simple set here to start.
BLOCKED_IPS: set[str] = set()


def _reject_if_blocked_ip(request: Request) -> None:
    client_id = _client_identifier(request)
    if client_id in BLOCKED_IPS:
        logger.warning("Blocked IP attempted auth request_id=%s ip=%s", _request_id(request), client_id)
...
async def login(request: Request):
    ...
    _reject_if_blocked_ip(request)
    _apply_auth_rate_limit(request=request, scope="auth_login", email=normalized_email, limit=5, window_seconds=900)
    try:
