...
export default function Logs() {
  ...
  useEffect(() => {
    if (!isLiveStream) {
      setPausedLogs(Array.isArray(logs) ? [...logs] : []);
    }
    else setPausedLogs([]);
  }, [isLiveStream, logs]);
  ...
  const displayedLogs = useMemo(() => {
    ...
    const safeSourceLogs = Array.isArray(sourceLogs) ? sourceLogs : [];

    return safeSourceLogs.filter((log) => {
      if (!log) return false;

      const logDateValue = log.timestamp || log.created_at;
      if (statusFilter && String(log.status) !== statusFilter) return false;
      if (threatTypeFilter && String(log.threat_type) !== threatTypeFilter)
        return false;
      if (apiKeyFilter && String(log.api_key_id ?? "") !== apiKeyFilter)
        return false;
      if (startTime) {
      ...
      if (term) {
        const hay =
          `${log.endpoint ?? ""} ${log.method ?? ""} ${log.threat_type ?? ""}`?.toLowerCase() || "";
  ...
  const handleCopy = (log: SecurityLog, logId: string) => {
  ...
  if (!Array.isArray(displayedLogs)) {
    console.error("Logs is not an array:", displayedLogs);
    return null;
  }

  if (isLoading && (!Array.isArray(logs) || logs.length === 0)) {
    return (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Real-time stream of AI gateway requests and threat detections.
          </p>
        </div>
        <div className="flex space-x-3">
          <Button
            ...
            className={
              isLiveStream
                ? "bg-red-900/50 text-red-400 border-red-800/50 hover:bg-red-900/70"
                : "text-slate-300"
            }
                        ...
                        <div className="p-4 pt-0 border-t border-white/5 mt-2 bg-slate-950/30 rounded-b-lg">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <Button
                              ...
                              onClick={(e: any) => {
                                e.stopPropagation();
                                handleCopy(log, logId);
                              }}
                              className="h-6 text-xs text-slate-400"
                            >
                              {copiedId === logId ? (
                                <Check className="h-3 w-3 mr-1 text-clean" />
                              ) : (
                                <Copy className="h-3 w-3 mr-1" />
                              )}
                              {copiedId === logId ? "Copied" : "Copy"}
                            </Button>
                          </div>
