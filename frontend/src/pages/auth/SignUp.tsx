...
import { Shield, Mail, Lock, ArrowRight, User, CheckCircle2 } from 'lucide-react';
import { Button } from '../../components/ui/Button';
import { setDisplayName } from '../../services/auth';
import {
...
export default function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInfo(String(data?.message || 'Verification email sent.'));
      setIsSubmitted(true);
    } catch (err: any) {
          ...
          {isSubmitted ? (
            <div className="text-center">
              ...
              <p className="text-sm text-slate-400 mb-6">
                We sent a verification link to <span className="font-medium text-slate-300">{email}</span>. Verify your account before signing in.
              </p>
              {info && (
                <div className="text-sm text-emerald-200 bg-emerald-900/20 border border-emerald-900/40 rounded-lg px-3 py-2 mb-4">
                  {info}
                </div>
              )}
              <Button
                ...
                className="w-full border-white/10 bg-slate-950/50 hover:bg-slate-800 text-slate-300 mb-3"
              >
